...
\item Python. This could be a viable alternative but would require additional time for the researcher to reach an adequate level of competence. Python offers a native library based on the FP-growth algorithm, which is significantly faster than \textit{apriori} which is an advantage over R. An FP-growth reference implementation is available to R but only via C integration.
\end{itemize}
...
The \textit{apriori} algorithm is provided to the R environment by the arules package, \cite{arulespackage}. This is found to lack some of the flexibility required to be easily repurposed for the exclusive discovery of CARs. It is likely that some workaround code needs to be generated to generate all rules then filter down to a valid set. This work requires a fast, efficient algorithm for mining CARs over a dataset for each variable containing any missing values. Developing this in R will also be left to future work.
