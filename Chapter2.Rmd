...
Most of these options have serious shortcomings. Accepting values on a continuous scale for data which is supposed to be ordinal may be entirely inappropriate if counts and tabulations are required. Three of the options are prediction methods which are valid only if the disadvantages described in Section \secref{sec:singimp} are acceptable. However, they could be used to generate multiple, non-deterministic values if applied to bootstrapped samples. 
...
* The choice of benchmark methods tailored for the Likert data type. These included Person Mean (PM), Correct Item Mean (CIM), Two-Way (TW) and Item Correlation Substitution (ICS) discussed in Section \secref{sec:singimp}. Multiple imputation was not used at all.
...
Much can be learned from the state of the art techniques. For example, the iterative chained equations approach can be adapted by imputing new values one variable at a time and simply substituting the regression step with a rule discovery step. This can be combined with a propensity step, which splits the dataset regressing a binary representation of presence or absence of data onto the other variables creates a separate imputation model for each split. @tan2005chapter7 details a number of other techniques used to enhance classification/prediction with association rules, and these could also be incorporated.
...
```{r concept-model, fig.cap='Conceptual model for the proposed ARM based imputation workflow.'}
knitr::include_graphics("concept_model.png", dpi = 100)
```

Figure \@ref(fig:concept-model) shows a conceptual model for the overall workflow required to implement ARM, incorporating the key learning from this literature review. This will become the foundation for the project's software artefact, the arulesimp package for R.