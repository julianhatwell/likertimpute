...
Alternative approaches also exist; @pantanowitz2008evaluating investigates the use of random forest, neural networks and genetic algorithms for imputation, but these are not as widely discussed elsewhere in the literature. 
...
Most of these options have serious shortcomings. Accepting values on a continuous scale for data which is supposed to be ordinal may be entirely inappropriate if counts and tabulations are required. Three of the options are prediction methods which are valid only if the disadvantages described in the previous section are acceptable. However, they could be used to generate multiple, non-deterministic values if applied to bootstrapped samples. 
...
* The choice of benchmark methods tailored for the Likert data type. These included Person Mean (PM), Correct Item Mean (CIM), Two-Way (TW) and Item Correlation Substitution (ICS) discussed in the previous section. **hyperlink or reference here** Multiple imputation was not used at all.
...
To frame this problem another way, on one hand a low support can cause a combinatorial explosion when mining for frequent patterns and AR. On the other, rules may have very similar confidence while having very disparate support scores. In the case of a near tie in confidence, what adjustments must be made for rules withvery high or very low support? The article goes on to propose a multi-faceted solution. This involves using the FP-growth algorithm for more efficient storage and retrieval of a larger rule set. Also importantly, @li2001cmar recommend making an aggregate prediction from a set of rules. This removes the problem of simply using the rule with the highest confidence. They use an adjusted chi square statistic, which was empirically shown to correct for bias towards the minority class.
...
@mutter2004classification goes on to propose alternative interestingness measure that meets this requirement, which is derived from support, confidence and lift:
...
A consequence of generating rules with a minimum support is that it is possible that not all instances are covered by a minimumally supporting rule. @liu2001classification deals with this by including a default class (usually the majority class) as the final rule, such that a CBA classifier always takes the form:

\begin{equation}
  <\ r_1 \succ r_2 \succ \dots \succ r_n \succ\ \{\} \implies \textit{default class}\ >
  (\#eq:cbaclassifier)
\end{equation}

where $\{\}$ signifies any other combination of attributes not found in a rule anticedent.
...
A particular characteristic of many surveys is the use of ordinal data and multiple-item (Likert) scales. @huisman1999missing states that there is a strong relationship between the individual items of a Likert scale which measure one latent trait. Techniques that can recognize this within-instance, structural information and preserve it in the imputed dataset should be valuable. The previous sections have shown how ARM finds significant, frequent patterns within the instances based on a minimum support and confidence. Filtering on the most significant rules, whose antecedent matches and instance with missing data and wholse consequent is the same variable with missing data, a potentially useful prediction can be made.

Ordinal and Likert scale data are not continuous or normally distributed and it is often meaningless use methods on these data types that return a non-integer value, such as the mean. MI also falls in to this category, as it delivers values based on MVN normal variables in multiple copies, which must also be averaged before any subsequent analysis. 

@agrawal1994fast states that association rules mining uses probabilistic measures (support and confidence) for discovering frequent patterns. Association rules mining algorithms work on discretized or categorical data, such as ordinal, nominal and binary. Furthermore, @chandola2005summarization describe association rules as a compact model of a dataset and as such may be used to enhance other stages of the analysis. So an association rules-based method for survey data would have several advantages over other imputation methods, yet a search of the literature yields no information on the use of association rules in this context.
