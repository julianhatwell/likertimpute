...
In contrast to grand mean, group mean or regression imputation, non-deterministic processes can be applied. The most basic of these is simple (naive) random imputation, whereby imputed values are generated by sampling with replacement from the observed data. Clearly this ignores any useful information from the other variables, which is undesirable. Hence methods have been developed that that do take into account information from all available variables. For example, @gelman2006data [p.536] proposes random regression imputation, which modifies the result of deterministic regression imputation by adding a random error drawn from a normal distribution with the residual standard error from the underlying linear model. It might also be valid to add a bootstrapped sample of the residual errors to the linear prediction.
...
Further to the formalization of measures of missingness in a dataset, @carpita2011imputation continues by defining a set of imputatation strategies reported to be particularly effective for Likert scales. Adapted from the same article, the specific case for Likert scales is given as a prelude to the definitions of the strategies themselves:
...
$$x_{ij} =
\begin{cases} 
k \in \{1,\ 2, \ \dots, \ K\ \},\ \text{if}\ m_{ij} = 1 \\
\text{missing}, \text{otherwise}
\end{cases}, \ m_{ij} \in \{0,\ 1\}$$

The person mean is: $$\bar{x_{i+}} = \frac{\sum_{j \in A(i)}x_{ij}}{p_{i+}}$$

The item mean is: $$\bar{x_{+j}} = \frac{\sum_{i \in B(j)}x_{ij}}{n_{+j}}$$
...
$$\hat{x_{ij}}^{(CIM)} = [\bar{x_{ij}}^{(CIM)}]  + \epsilon_{v} = [\frac{\bar{x_{i+}}}{\bar{\bar{x_{i+}}}} \bar{x_{+j}}] + \epsilon_{v},\ \epsilon_{v} \sim B(1, [\circ]\bar{x_{ij}}^{(CIM)})$$
...
There are also non-parametric alternatives of single imputation. Many are best suited for specific situations. For example, "last value carried forward" is only applicable in longitudinal, or time-based surveys. It can result in erratic reeadings for trends analysis. For missing data in a nominal variable it is sometimes acceptable simply to add a new category, "missing." Logical rules, survey structure or domain knowledge can be applied. For example, a respondent reporting low or moderate drinking might have the number of alcohol units per week imputed if missing. Similarly, someone reporting they worked zero hours per week might be also assigned an income of zero. @wu2015comparison mentions the use of a random forest in the literature which is explored because it can be applied to all kinds of categorical data, requires no distributional assumption and can model multi-way interaction. It is implemented in R in the missForest package, @R-missForest.

The most important non-parametric imputation methods are generally referred to as "similar pattern response matching" [@joreskog2005structural p.3] and also Hot-deck imputation.  @gelman2006data seems to refer to Hot-deck and matching interchangeably, which seems reasonable given the descriptions of these elsewhere in the literature. @joreskog2005structural defines the intuition behind matching imputations as follows; If an individual with missing variable *x* has same response pattern in other variables as another case, then it's likely that variable *x* also matches. The article goes on to say that if multiple individuals are found with the matching response pattern then that's even stronger evidence for value *x*. @gelman2006data [p.538] describes the process as a "non-parametric version of regression" but it is not clear what the grounds are for this description. Certainly the method is useful where (linear) regression is challenging. Only @bono2007missing seems to differ, stating that the substitute value "may be selected simpy at random or using an elaborate scheme," the latter referring to similarity with other cases. This definition does not seem to be entirely correct in that the random case would be no different to naive random imputation and wouldn't use any information from the other cases. In fairness, the author quite rightly points out the main disadvantages with the method, which are defining and programmatically configuring the customized lookup and selection process for each variable.

It should be noted that Cold-deck imputation is also mentioned in the literature, but only as a brief aside [@kowarik2016imputation; @bono2007missing; @gelman2006data]. In this process, substitute values are found by reference to prior studies, not the same dataset where the missing values are found. The names refer to old punch-card computer systems. The cold-deck was a previous run, while the Hot-deck was the current run literally still hot out of the card-reader. Cold-deck imputation is barely even mentioned, perhaps because it is just a historical curiosity and not considered useful. This is not made clear in any of the sources. The literature generally focuses on statistical and computational techniques.
...
@graham2007many states that MI and FIML (full information maximum likelihood) are the two most common techniques used with missing data, but FIML is far more computationally expensive. @wu2015comparison lists a number of other disadvantages of FIML so for the purpose of this research, only MI will be considered to be the state of the art. Different authors refer to EM, data augmentation and Gibbs sampling underpinning MI but @wu2015comparison argues that all these are special cases of MCMC, so it would be fair to say that MI is a MCMC process, whatever the specific implementation.

** review the distinction between single and multiple - iterative process and MI **
@wu2015comparison "The univariate approach is also called chained equation or sequential regression equation imputation because the variables are imputed separately and sequentially at each iteration." This is MICE - flexible to accomodated different data types.



To begin, a random starting value is set for all missing values by sampling with replacement from the observed values or randomly drawn from a posterior distribution. Then new values are imputed one variable at a time, usually in order of ascending missingness, using all other values. When all variables have been imputed, this completes a cycle. For the first cycle, observed and initialized values are used. In subsequent cycles, observed and currently imputed values are used. The process iterates until convergence, i.e. the currently imputed values no longer change from one cycle to the next. After convergence, the model can be used to save imputations at specified intervales to avoid autocorrelations. The Amelia II package implements EM with boostrapping, in which boostrapped samples are used to generate the EM estimates of the prediction parameters. This can run in parallel and each bootstrap can generate separate, uncorrelated imputations.

@wu2015comparison strategies for applying this to ordinal. Assume normality and accept imputation values on a continuous scale. Either use them raw or round them. Or use MICE with discriminant analysis to predict as categorical, or use multinomial or prop. odds. Prop odds assumes predictors have same influence over all categories, multinom does not. Prop. odds has fewer parameters. Finally can use latent variable strategy - predict as is normal then discretize based on threshold parameters.

@white2011multiple explains how MICE uses EM to generate a multivariate normal model, in the form of a posterior distribution of the model parameters conditional on the observed data. Up to this point, the process is similar to the EM process described for single imputation. The multiple imputation procedure is to take random draws are taken from this distribution to create a predictive model (usually a linear regression) which is then used to predict values for the missing data in the dataset, one variable at a time. The imputed values are introduced to the predictive model as it progresses over each variable. The predictions are repeated iteratively over each variable until the model converges or the maximum number of iterations is expended. The random draw and iterative prediction process are repeated $m$ times, to create $m$ completed datasets, differing only in the values of the substituted variables. The name multiple imputation actually refers to the $m$ imputations, but the literature contains many variations, referring sometimes to the EM algorithm, the multivariate model, the possibility of imputing more than one missing variable or the iterative process used to converge each of the $m$ imputations. It is easy to see why the literature is inconsistent on this matter!

@schafer1997analysis cited in @wu2015comparison describes the MI process as being made up of two steps. The posterior P-step and the imputation I-step. A dataset $D$ is comprised of observed data $D_{obs}$ and missing data $D_{mis}$. On iteration step $t$, P-step is to a random set of imputation are drawn parameter values $\theta^t$, is drawn from the posterior distribution of $\theta$ which is typically a vector of means and the corresponding covariance matrix.

$$\theta^t \sim P(\theta \mid D_{obs}, \ D^{t-1}_{mis}) $$

I-step is make a draw random for new values for $D^t_{mis}$ from the predictive distribution.
$$D^t_{mis} \sim P(D_{mis} \mid D_{obs}, \ \theta^t)$$

In the iterative univariate approach, the same process is followed for one variable at a time. Instead of two steps there are 2p steps where p is the number of variables.


Any further statistical analysis should be performed on the each $m$ imputations and pooled. @graham2007many ** more  ** reports that the resulting estimates are unbiased. Furthermore, the variance in estimates attributable to the missing data can also be easily estimated using Rubin's rules. 

The following example is adapted from @graham2007many and @leite2010performance. Let $q$ be a paremeter of interest, to be estimated from $m$ imputations.

The point estimate be calculated as $\bar{q} = \frac{1}{m} \sum^m_{i = 1}\hat{q_i}$

The within imputation variance $W_q = \frac{1}{m} \sum^m_{i = 1} SE^2_{\hat{q_i}}$

The between imputation variance $B_q = \frac{1}{m-1} \sum^m_{i = 1} (\hat{q_i}-\bar{q})$

The total imputation variance $T_q = W_q + (1 + \frac{1}{m})B_q$ and standard error $SE_q = \sqrt{T_q}$ which can be used for calculating the t-value of the parameter estimate and degrees of freedom is calculated as $df = (m-1)(1+\frac{mW_q}{m+1}B_q)^2$

Another important formula in MI is the fraction of missing imformation (FMI). Although the proportion of missing data $\frac{|D_{mis}|}{|D|}$ may be useful in the simplest of cases, it doesn't account for increased information available if variables are highly correlated. FMI is estimated for each parameter that has been estimated from MI, using the specific $W$ and $B$ for that parameter estimate.

Let $FMI = \gamma = \frac{\frac{r+2}{df + 3}}{r+1}$ where $r = \frac{1 + \frac{1}{m}B}{W}$

FMI is used to determine the appropriate number for $m$, under the theory established by @rubin1987multiple cited in @graham2007many and @schafer1997analysis cited in @graham2007many. These authors are cited very widely and are conferred a great deal of authority by other authors. They suggest that $m$ = 3-5 is usually enough imputations. Nevertheless, there are some disagreements; @leite2010performance reports that $m$ = 5-10 is standard. It is not clear why there is this difference as the same secondary sources are cited. @wu2005role states that there is no support in the literature for the $m$ = 5 and that one imputation is often enough, but during this review it was the only article found to make the case that MI methods should be used for single imputation! A non-academic source was found which only used $m$ = 1 but this appeared to be a misunderstanding of the method. In stark contrast, @graham2007many demonstrates that $\gamma$ is poorly estimated for small values of $m$ using Monte-Carlo simulations and posits that 3-5 is nowhere near enough. In particular, they show that statistical power is very sensitive to small values of $m$. This is a crucial point because preserving statistical power is one of the major reasons for not using a simpler technique, such as list-wise deletion. @white2011multiple goes further, also using Monte-Carlo simulations, stating that results are not reproducible when $m$ is too low. The latter ran experiments showing a loss of precision of 3% which could make the difference between a p-value of 0.03 and 0.08, depending on the imputed values in the $m$ imputations. The authors cite other experiments that show a clear link between confidence interval widths and the fraction of missing information in tne dataset. As a result, they endorse the suggestion of 1 imputation per 1% of missing data. From the variety of experimental and formal evidence, it should be concluded that this is a valuable rule of thumb. With the power of modern computing today, it seems unnecessary to stick to a bare minimum number of runs for anything other than massive datasets with high incidence of missing data, @azur2011multiple. It should be fairly trivial to increase $m$ and there are no counter-indications for doing so.
...
Combined multiple impute equations on gelman p. 542
and White on page 2


@wu2015comparison [p.6] calls it "fully conditional imputation because prediction of missing data on one variable is conditional on the current values of all of the other variables at a specific iteration." @white2011multiple [p.378] call this "proper imputation, because it incorporates all sources of variability and uncertainty in the imputed values, including prediction errors of the individual values and errors of estimation in the fitted coefficients of the imputation model." @leite2010performance [p.65] states that "restores two sources of variability: the variability of each variable and the variability of the sample covariance matrix." A multivariate regression model is generated for each variable $z$ in turn, resulting in $\hat{\beta}$ and $V$, the vector of regression coefficients and corresponding covariance matrix. Let $\beta^*$ be a random draw from $\beta^* \sim MVN(\hat{\beta}, V)$. Imputed values for $z$ are drawn from this posterior prdictive distribution. MOSTLY QUOTED

@white2011multiple describes how MICE handles different variable types. Handles non-normality by transformation or **predictive mean matching**. This is important because it is a concession that non-normality has to be addressed with a non-parametric process!
...
From mi package vignette @su2011multiple
The predictive mean matching method (mi.pmm()) works in the following way. For each observation
with a missing value on a given variable, we find the observation (from among those
...
carpita - approximate bayesian bootstrap. Draw a bootstrap sample from the observed data, then from this, draw with replacement some values for the imputation. Repeat for mulitple imputamtion, Do so within calsses for the Approximate bayesian bootstrap with propensity. They propose novel technique ABPN appox bayes boot with propensity and  nearest naightbour. Focus on likert byt say it could be used elsewhere. QUOTE  The bootstrap step allows to sample the parameters of the empirical distribution of the observed data before drawing from the posterior distribution of the incomplete data. QUOTE The propensity step is a logistic regression (L step) fo teh predictor variables against missing / non- missing - from a balanced sample. Sort by propensity into classes (P step) into n classes (carpita chose 4) Draw a bootstrap sample from within each class.
Nearest neighbour N step: A fraction of complete cases from teh same class are taken,the nearest naighbours are found and a (ABB step) bootrap sample of these is taken. The donor case is randomly taken from these. Update the non-missing from the donor case.
...
The coefficient of correlation was measured in this experiment using Fisher's r-to-$Z_r$ transformation which corrects for non-normality in sampling distributions: 
...
Synthetic data was generated from two underlying (latent) variables with loadings or 0.7, to yield 12 ordinal variables in two six-item scales, A and B. The mean within item correlation was 0.49 and between items was 0.15. Skewness was introduced by selection of appropriate thresholds for discretization accoding to the method in @rhemtulla2012can cited in @wu2015comparison. A single large dataset (N=500000) was generated to estimate the population parameters. The reasoning behind this was that the parameters cannot be analytically determined after the discretization process. This seems to be a much more pragmatic approach and gives an improved representation of Likert scales data than the one used in @leite2010performance. 

**put the thresholds used in the my appendices**

To create a MAR process, two auxiliary, continuous variables were created and used to generate the missing values on the ordinal variables. Aux 1 was rank ordered and assinged a probability of $P = 1 - \frac{rank(Aux1)}{N}$. Values of A1-3 were set to missing if the probability of random variable $X \sim \mathcal{U}(0,\ 1) < P$. A similar process was carried out for variables B1-3. They were set to missing if $X < P = 1 - \frac{rank(Aux1 + Aux2)}{N}$ and the process was stopped after the desired missing proportion was reached.
...
Where $\mathcal{F}$ is the F distribution, $\gamma$ is the desired significance level, $n$ is the sample size, $p$ is the number of items, $df_1 = (n-1)$ and $df2 = (n-1)(p-1)$.
