...
Ordinal data generally take the form of a categorical variable with levels $k \in \{0,\ \dots,\ K\}$ or $k \in \{1,\ \dots,\ K\}$ or sometimes a symmetric form $k \in \{-K,\ \dots,\ 0,\ \dots,\ K\}$ and all are interpreted as a monotonic, ranking representation $Least \succ Less \succ More \succ Most$. Ordinal variables of type AOCV can be used individually but are often combined in a survey as multiple related items (multiple-item response scales). Likert scales are a special case of ordinal variable and a special case of multiple-item response, whereby a collection of statements are scored by the respondent using an integer series which is anchored to phrases of sentiment or attitude (e.g. 1 = strongly disagree, 5 = strongly agree). The collection of responses measure the outwardly manifested facets of some latent concept or factor. The respondents' positions on the latent factor can then be estimated by summing or averaging the multiple responses of the individual items [@carpita2011imputation]. Likert scales are ubiquitous in behavioural science, marketing, usability, customer feedback, psychological and clinical research. Although the term Likert scale is sometimes used in the literature to refer to single items following the classic agree/disagree scoring, the correct definition refers to a combined scale as described here. A thorough treatment of Likert scales is given in @gliem2003calculating.
...
Both types of non-response have serious consequences. However, this work is concerned only with item non-response which will be the focus for the remainder of this section, unless otherwise stated.
...
\begin{equation}
  X = \{x_{ij} = k\ :\ i \in \{1,\ 2,\ \dots,\ n\}\ ,\ j \in \{1,\ 2,\ \dots,\ p\}\ ,\ k \in \{1,\ 2,\ \dots,\ K,\ missing\}\ \}
  (\#eq:carpitaX)
\end{equation}
...
This informative example shows how the mean and variance of the MCAR data is almost identical to the underlying distribution. Using mean imputation gives a mean which is identical to the MCAR data to many decimal places, as can be seen in Table \@ref(tab:sim-grandmean-imp-means). However, the variance is much reduced in the imputed dataset, as shown in Table \@ref(tab:sim-grandmean-imp-vars). Density plots of selected bootstrap iterations show very clearly what has happened. See Figure \@ref(fig:sim-grandmean-imp-plot). Not shown here, but also notable, is that bivariate correlations would also be artificially reduced by using a constant value to replace missing data.

Mean imputation, or any other technique that relies only on a measure of central tendency is too simplistic an approach for many missing data problems. Aside from the issues demonstrated here, the method takes no information from other variables if they are available. If data are MAR, then a dependency between the missingness and the observed data has been established and should be exploited. Other simple, parametric imputation methods that do exploit this information include group mean imputation. This is where the mean is stratified over one or more categorical variables. Regression imputation is better still, generating estimated values from a predictive model (usually linear, or whatever is appropriate for the data).
...
@graham2007many states that MI and full information maximum likelihood (FIML) are the two most common techniques used with missing data, but based on this literature review, FIML is discussed very rarely in comparison to MI. @graham2007many also states that FIML is far more computationally expensive. @wu2015comparison lists a number of other disadvantages of FIML. For these reasons, only MI will be considered in this discussion of the state of the art. Different authors refer to EM, data augmentation and Gibbs sampling as the underpinnings of MI but @wu2015comparison argues that all these are special cases of MCMC, so it would be fair to say that MI is a MCMC process, whatever the specific implementation.
...
After MI, any subsequent statistical analysis should be performed on each of the $m$ imputations and pooled. Many authors [@rodwell2014comparison; @leite2010performance; @graham2007many; @donders2006review] report that the resulting estimates are unbiased. Furthermore, the variance in estimates attributable to the missing data can easily be estimated using Rubin's rules, which are recapitulated widely in most sources that discuss MI in any detail. The following definitions are adapted from @graham2007many, @leite2010performance and @white2011multiple:
...
Likert scales are not specifically mentioned they are obviously covered by several of the arguments given above. @plumpton2016multiple goes on to say that MI is robust to departures from normality and argues for the method's suitablility for many non-normal situations, including on Likert scales. However, it is difficult to see how these challenges are overcome using the classic MI method based entirely on a MVN model. Indeed, @shrive2006dealing describes an experiment using real-world data from a self-reported depression study that shows Person Mean imputation doing slightly better than MI in those scenarios that diverged the most from normality.
...
@jonsson2004evaluation shows that Hot-decking methods, (which covers the association rules based method under investigation) do not always find a suitable donor value. This means that a final round of listwise deletion of remaining incomplete cases may still be necessary, with all the inherent assumptions and risks of such action. Either that, or a default class substitution must be performed on the remaining missing values. These factors suggest that the succuss, or otherwise, of an association rules based imputation method also needs take into account the ability to find estimated values. 
...
Testing the performance of imputation on entirely real-world or case study examples is intractable because the true value of population parameters and the underlying mechanisms of missingness are unknown prior to starting the study. Most such experiments with real world data have to make compromises by creating semi-simulated data. This is done by inferring a MVN model using the means and a covariance matrix from the observed data portion of the dataset and drawing simulated samples from this model. Again, this introduces design bias. Firstly, the MVN problem is the same as in the synthetic data case. Secondly, if the data is MAR or MNAR then the simulated data set will no longer be representative.

For synthetic data, it becomes a question of appropriate modeling and representation. A very pragmatic and simple process for creating rather realistic five-point Likert-scale simluations is given in @de2010five. This method simply gives proportions of each of the five possible scores in a population for different levels of simulated agreement, neutrality and disagreement, which empirically deliver predictable means, standard deviations, and levels of skewness and kurtosis. The main disadvantage with this method is that it lacks a means to fine tune the inter-item correlations. It is surprising that this method is not seen elsewhere in the literature. See \secref{sec:syndatfun} for examples.
...
bs_design_matrix <- knitr::kable(data.frame(
  ...
  , Options = factor_options
  , check.names = FALSE)
        , caption = "Experimental Design Matrix: Conditions. Adapted from source: Wu et al. (2015)")
...
where $q$ is the number of replications of the experiment and $s_{\theta}$ the empirical standard deviation of $\theta$ across all replications.
...
The Scale Score Error was also measured. Let $A'(i)$ be the set of indices of non-available items for subject $i$ and $B'(j)$ be the set of indices non-available respondents for item $j$. Consider the total score with complete, untreated data $x_{i+}$ and with imputed data with method \*, $\hat{x}^{(*)}_{i+}$ For subject $i$ the scale score error (SSE) is:
\begin{equation}
...
@mutter2004classification goes on to propose an alternative interestingness measure that meets this requirement, which is derived from *Support*, *Confidence* and *Lift*:
...
where $Y_i$ is an instance of $Y$ having a class label $i$ and $Y_{true}$ is the true class of $Y$. In addition, more general rules are preferred for prediction. This allows for pruning of redundant rules and more compact models. A rule is more general than another rule if it predicts the same instances correctly with a shorter antecedent. Refer to @mutter2004classification for the full definition and proof. Unfortunately this measure is not trivial to use for imputation because it requires the true classes to be known (as they would be in the predictive training set). For imputation, the true classes are unknown

Pruning strategies are essential for the good performance of an AR based classifier. The most popular approach is database coverage or the covering method, which ensures that each rule can classify at least one training instance correctly and each instance can be classified by the best rule, according to prediction accuracy, or interestingness measure. @liu2001classification describes this as a greedy algorithm. It starts with an exhaustive search of all the rules and works from the highest ranking rule to the lowest, removing instances from the training set when a covering rule is found. More specific rules are required to cover the most difficult to classify instances. Another method, CPAR [@yin2003cpar] downweights instances as covering rules are found, rather than removing them. This echoes the boosting method.

Rules are ranked according to a simple set of rules. In the method outlined in @liu2001classification [p.5], $r_i \succ r_j$ if:
...
This can naturally result in some instances being covered by more than one rule. So an AR-based prediction model also requires a way to resolve multiple different predictions. @mutter2004classification proposes majority vote with weighting, the weights being determined by the rank order of each rule with respect to interestingness measures. This scheme allows comparison between different algorithms in an experimental setting. An alternative is simply to take the prediction from the top ranking rule.
...
While prediction and imputation differ in scope and goal, the intention of finding estimated values rather than accurate values could be seen as a mere relaxation of success criteria. If prediction with excellent results is possible from association rules, then it stands to reason that imputation in some form must also be possible. This research then will use rules found in the observed data and apply the techniques of prediction described here to create an algorithm for estimating the missing values in the data.

## Summary of the Literature Review {#sec:chp2sum}

An imputation technique for survey data is a special case. As discussed in \secref{sec:charsurv}, there are many arguments against using standard procedures because the assumptions of normality and continuous data are violated. It is often meaningless use methods such as mean imputation on these data types that return a non-integer values. MI, the state of the art for imputation, falls in to this category. It delivers values based on MVN normal variables in multiple copies, which must also be averaged before any subsequent analysis. 
