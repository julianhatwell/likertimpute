...
Association rules mining (ARM) was first popularised by the development of a fast algorithm for large databases, *apriori*, in @agrawal1993mining. It falls under the general banner of data mining for its ability to provide a compact and interpretable summary of large amounts of observed data. This is achieved by extracting frequently occuring patterns. It was developed in response to the vast amount of data collected by large retail enterprises on their customer purchase data.
...
The brute force method of computing all possible rules in a transaction set of n possible items is $O(n2^{n-1})$ for binary attributes (i.e. present/absent) and $O(m^n)$ where attributes can take on $m$ possible values (as in a Likert scale). @mutter2004classification states that it is an intractable problem to find all rules in even moderately sized databases. When it emerged, *apriori* was described as a "fast" algorithm because it outperformed everything that came before.

*apriori* is still widely used today. @tan2005chapter6 explains that to achieve its objective of efficiently finding useful association rules, ARM must solve two distinct problems:
...
These challenges are addressed under the *apriori* algorithm by exploiting the downward closure principle which states that an itemset is frequent $iif$ all of its proper subsets are frequent. *apriori* searching the database iteratively, starting with single items that occur more frequently than a given probabilistic threshold, known as *support*. Rule discovery is deferred until all frequent itemsets are found i.e those occuring with > minimum support.
...
Pruning strategies are essential for the good performance of an AR based classifier. The most popular approach is database coverage, which ensures that each rule can classify at least one training instance correctly and each instance can be classified by the best rule, according to prediction accuracy, or interestingness measure. @liu2001classification describes this as a greedy algorithm. It starts with an exhaustive search of all the rules and works from the highest ranking rule to the lowest, removing instances from the training set when a covering rule is found. More specific rules are required to cover the most difficult to classify instances. Another method downweights instances as covering rules are found, rather than removing them. This echoes the boosting method.
...
There are alternative algorithms for ARM-based classification. CPAR, presented in @yin2003cpar implemented an efficient rule induction algorithm but does not appear very widely in the literature. CMAR [@li2001cmar] implements an efficient FP-growth tree for rule storage. Furthermore, multiple association rules are considered predict according to a weighting scheme.
